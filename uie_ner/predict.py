...
class UIEPredictor(object):
    ...
    def __init__(self, model, task_path=None, schema_lang="zh", engine='pytorch', device='cpu', position_prob=0.5, max_seq_len=512, batch_size=64, split_sentence=False, use_fp16=False):
...
def predict(args):
    ...
    ie = UIEPredictor(model=args.model, task_path=args.model, schema_lang='zh',
                      engine='pytorch', device=args.device,
                      position_prob=args.position_prob, max_seq_len=args.max_seq_len, batch_size=16,
                      split_sentence=False, use_fp16=False)
    for task_name, task_data_list in data.items():
        for line in task_data_list:
            prompt = ['被告人交通工具', '被告人交通工具情况及行驶情况', '被告人违规情况', '行为地点', '搭载人姓名', '其他事件参与人', '参与人交通工具',
                      '参与人交通工具情况及行驶情况', '参与人违规情况', '被告人责任认定', '参与人责任认定', '被告人行为总结']
            p_lab = {}
            ie.set_schema(prompt)
            result = ie(line['context'].strip())
            entities = []
            record = []
            for entity_type in prompt:
                ...
                for samp in result[0][entity_type]:
                    if entity_type not in record:
                        record.append(entity_type)
                        entities.append({'label': str(label2id[entity_type]), 'span': [[samp['start'], samp['end']]],
                                     'text': [line['context'][samp['start']:samp['end']]]})
                    else:
                        for item in entities:
                            if item['label'] == entity_type:
                                item['span'].append([samp['start'], samp['end']])
                                item['text'].append(line['context'][samp['start']:samp['end']])
            output.write(json.dumps({'context': line['context'], 'id': line['id'], 'entities': entities},
                                ensure_ascii=False) + '\n')
